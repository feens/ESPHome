...
static const char *const TAG = "cooktop_plug";

void CooktopPlugComponent::loop() {
...
float CooktopPlugComponent::get_setup_priority() const { return setup_priority::DATA; }

bool CooktopPlugComponent::check_byte_() {
...
void CooktopPlugComponent::parse_data_() {
...
//   uint32_t current_cycle = this->get_24_bit_uint_(11);
  ...
  if (power_cycle_exceeds_range) {
    ...
    // if (power > this->power_threshold_ && this->previous_power_ <= this->power_threshold_) {
        this->power_sensor_->publish_state(power);
    // } else if(power <= this->power_threshold_ && this->previous_power_ > this->power_threshold_) {
        // this->power_sensor_->publish_state(power);
    // }

    this->previous_power_ = power;
  }
...
uint32_t CooktopPlugComponent::get_24_bit_uint_(uint8_t start_index) {
...
void CooktopPlugComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "CooktopPlug:");
